...
import (
	...
	"errors"
	"fmt"
	"github.com/vsoch/gosmeagle/pkg/archive"
	"github.com/vsoch/gosmeagle/pkg/debug/dwarf"
	"github.com/vsoch/gosmeagle/pkg/goobj"
	"github.com/vsoch/gosmeagle/pkg/objabi"
	"github.com/vsoch/gosmeagle/pkg/sys"
	"io"
	"log"
	"os"
)
...
func (f *goobjFile) Symbols() ([]Symbol, error) {
	r := f.r
	var syms []Symbol
	...
	for i := uint32(0); i < ndef; i++ {
		...
		sym := ElfSymbol{
			Name:    name,
			Address: uint64(r.DataOff(i)),
			Size:    int64(osym.Siz()),
			Code:    code,
		}

		relocs := r.Relocs(i)
		sym.Relocations = make([]Relocation, len(relocs))
		for j := range relocs {
			rel := &relocs[j]
			sym.Relocations[j] = Relocation{
				Address: uint64(r.DataOff(i)) + uint64(rel.Off()),
				Size:    uint64(rel.Siz()),
				Stringer: goobjReloc{
					Off:  rel.Off(),
					Size: rel.Siz(),
	...
	for i := ndef; i < n; i++ {
		osym := r.Sym(i)
		sym := ElfSymbol{Name: osym.Name(r), Code: 'U'}
		syms = append(syms, &sym)
	}
	for i := 0; i < nrefName; i++ {
		rn := r.RefName(i)
		sym := ElfSymbol{Name: rn.Name(r), Code: 'U'}
		syms = append(syms, &sym)
	}
...
func (f *goobjFile) ParseDwarf() map[string]map[string]DwarfEntry {
	dwf, err := f.Dwarf()
	if err != nil {
